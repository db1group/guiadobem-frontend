...
import imgTriste from "../imagens/cara-triste.png";

import "./ListaEstabelecimento.css";

export default class ListaEstabelecimentos extends Component {
  PLANOFUNDO = ["bkgHortiFruiti", "bkgConfeitaria", "bkgServicos", "bkgOutros"];
  ...
  async componentDidMount() {
    ...
    this.setState({
      textoTitulo: `${
        responseCidade.data.nome ? textoTitulo : responseCategoria.data.nome
      } `,
    });
  ...
  atualizarEstabelecimentosFiltrados = () => {
    ...
    const estabelecimentosFiltrados = estabelecimentos.filter(
      (estabelecimento) => {
        return (
          possuiTexto(estabelecimento.nome, textoPesquisado) ||
  ...
  render() {
    ...
    return (
      <Container>
        <Cabecalho />
        <Container>
          <Titulo botaoVoltarClique={history.goBack}>{textoTitulo}</Titulo>
          {this.renderFiltro()}
        </Container>
        {this.renderSemEstabelecimentos()}
        <CardColumns className="cardColumnsEstabelecimentos">
          {estabelecimentosFiltrados.map((estabelecimento) => (
            ...
            >
              <Card.Body>
                <Card.Title>
                  {this.renderTitulo(estabelecimento.nome)}
                </Card.Title>
  ...
  renderTitulo = (texto) => {
    ...
    return (
      <span style={{ fontWeight: "bold" }}>
        {this.destacarTexto(texto, textoPesquisado)}
      </span>
    );
  };
  ...
  renderCorpo = (estabelecimento) => {
    return (
      <>
        <div className="tipo-texto-card">
          {this.renderTexto("tipoproduto", estabelecimento.tipo)}
        </div>
        <div className="tipo-texto-card">
          {this.renderTexto("telefone", estabelecimento.telefone)}
        </div>
        <div className="tipo-texto-card">
          {this.renderTexto("whatsapp", estabelecimento.whatsapp)}
        </div>
        <div className="tipo-texto-card">
          {this.renderTexto("responsavel", estabelecimento.responsavel)}
        </div>
      </>
    );
  };
  renderTexto = (tipo, texto) => {
    ...
    return (
      <div>
        <div>
          {tipoTexto.icone}{" "}
          <span style={{ fontWeight: "bold" }}>{tipoTexto.rotulo}</span>
        </div>
  ...
  destacarTexto = (texto, destaque) => {
    ...
    const partesSemAcento = textoSemAcentos.split(
      new RegExp(`(${destaqueSemAcentos})`, "gi")
    );

    let indiceAtual = 0;
    const partes = partesSemAcento.map((parteSemAcento) => {
  ...
  renderSemEstabelecimentos = () => {
    const { estabelecimentos, carregando } = this.state;
    if (!carregando && estabelecimentos.length === 0) {
      return (
        <Container className="texto-sem-cadastro">
          <img src={imgTriste} alt="Triste sem cadastros" />
          <span>
            <h6>
              Poxa, que pena, não há nenhum cadastro para esta categoria por
              aqui!
            </h6>
            <br></br>
            Mas você pode mudar isso: ajude, indique e cadastre seu comércio ou
            o comércio de seu vizinho, e ajude a deixar este projeto ainda mais
            completo!
          </span>
        </Container>
      );
